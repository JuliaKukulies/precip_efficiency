...
times = np.arange(85)
path = Path('/glade/derecho/scratch/kukulies/idealized_mcs/19_2011-07-13_CTRL_Midwest_-Loc1_MCS_Storm-Nr_JJA-8-TH5/12000/') 
...
for fname in files:
    ...
    total_ice = total_ice.mean('south_north').values
    total_liquid = total_liquid.mean('south_north').values
    ...
    total_sublimation = ds.PRS_SDE.where(ds.PRS_SDE < 0, 0) + ds.PRS_IDE.where(ds.PRS_IDE < 0,0) + ds.PRI_IDE.where(ds.PRI_IDE < 0,0) + ds.PRG_GDE.where(ds.PRG_GDE < 0,0) + ds.PRI_INU.where(ds.PRI_INU < 0,0) + ds.PRI_IHA.where(ds.PRI_IHA < 0,0)
...
data.to_netcdf('/glade/derecho/scratch/kukulies/idealized_mcs/idealized_mcs_19_vertical_profile_12000.nc') 
